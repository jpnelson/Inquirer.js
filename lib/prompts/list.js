...
Prompt.prototype.onKeypress = function (input) {
  ...
  // var nonJkChars = prompt.autoComplete.split('').filter(function (char) {
  ...
  prompt.autoComplete += input.value;
  prompt.autoCompleteTimeout && clearTimeout(this.autoCompleteTimeout);
  this.autoCompleteTimeout = setTimeout(function () {
    prompt.autoComplete = '';
  }, 500);

  var matches = this.opt.choices.choices.filter(function (choice) {
    return choice.short && !choice.disabled;
  }).map(function (choice) {
    return choice.short.toLowerCase();
  }).map(function (short, i) {
    if (short.indexOf(prompt.autoComplete) === 0) {
      return i;
    }
    return -1;
  }).filter(function (x) {
